...
const emit = defineEmits(["delNode"]); //获取父组件的方法emit
import AddNodeApprover from "@/components/Generator/add-node-approver.vue";
...
const text = ref("请选择");
const titlebg = ref("background: rgb(87, 106, 149);");
const nodetype = ref("发起人");
const content = ref("选择审批人");
const icon = ref(
...
const setText = () => {
  switch (nodeValue.value.type) {
    case "start":
      content.value = "所有人";
      break;
    case "approver":
      nodetype.value = "审批人";
      titlebg.value = "background: rgb(255, 148, 62);";
      icon.value =
    ...
    case "notifier":
      nodetype.value = "抄送人";
      titlebg.value = "background: rgb(50, 150, 250)";
      icon.value =
  ...
  if (nodeValue.value.properties && nodeValue.value.properties.actionerRules) {
    var acr = nodeValue.value.properties.actionerRules[0];
    switch (acr.type) {
      ...
      case "target_approval":
        ...
        for (var i = 0; i < acr.approvals.length; i++) {
          content.value += acr.approvals[i].userName + "、";
        }
        content.value = content.value.substr(0, content.value.length - 1);
        break;
      case "target_label":
        nodetype.value = acr.labelNames;
        content.value = acr.labelNames;
        icon.value =
          "M644.8 581.568l160.64 187.456A64 64 0 0 1 756.842667 874.666667H267.157333a64 64 0 0 1-48.597333-105.642667l160.661333-187.434667c18.922667 11.52 39.466667 20.629333 61.205334 26.944L267.157333 810.666667H480v-128h64v128h212.842667l-173.269334-202.133334a254.613333 254.613333 0 0 0 61.226667-26.965333zM512 149.333333c117.824 0 213.333333 95.509333 213.333333 213.333334s-95.509333 213.333333-213.333333 213.333333-213.333333-95.509333-213.333333-213.333333S394.176 149.333333 512 149.333333z m0 64a149.333333 149.333333 0 1 0 0 298.666667 149.333333 149.333333 0 0 0 0-298.666667z";
        if (nodeValue.value.type === "approver")
          content.value += acr.actType === "and" ? "会签" : "或签";
        break;
      default:
    }
  }
