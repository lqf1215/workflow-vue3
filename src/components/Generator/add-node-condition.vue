...
const type1 = ref("dingtalk_actioner_range_condition");
watch(
  () => props.show,
  (newValue, oldValue) => {
    show1.value = newValue;
  }
);
watch(
  () => show1.value,
...
const save = () => {
  if (properties.conditions[0].length === 0) {
    alert("条件不能为空");
    return;
  }
  var flag = properties.conditions[0].every((cond) => {
    return ifValid(cond);
  });
  // console.log(flag)
  if (flag) {
    show1.value = false;
    emit("on-success", properties);
  }
};
const addCondition = () => {
  properties.conditions[0].push({
    pos: properties.conditions[0].length + 1,
    type: type1.value,
  });
};
const delGroup = (index) => {
  properties.conditions[0].splice(index, 1);
};
</script>
